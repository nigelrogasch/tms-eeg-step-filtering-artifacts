...
for idx = 1:length(id)
    for sx = 1:length(stepLevel)
        ...
        EEGnew = tesa_robustdemean(EEGnew,[10,1499],3);

        % Perform robust demean
        EEGnew = tesa_robustdemean(EEGnew,[-1500,-2],3);
...
% Save the data
save([pathData,'real_filter_butter_modified_demean'],'rmse','stepLevel','eegraw','eegfilt','eegfiltband','time','tep_gt','time_gt');
