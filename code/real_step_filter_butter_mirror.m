...
for idx = 1:length(id)
    for sx = 1:length(stepLevel)
        ...
        % Filter the data
        x_filt = tesa_highpass_edge_step(EEGnew.data, EEGnew.srate, 'RemoveStep', true, 'RemoveTrend', true,...
            'FilterOrder',2,'PadLength',1,'StepIndex',stpi);
...
% Save the data
save([pathData,'real_filter_butter_mirror'],'rmse','stepLevel','eegraw','eegfilt','eegfiltband','time','tep_gt','time_gt');
