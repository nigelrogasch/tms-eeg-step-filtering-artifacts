...
% Load simulated TEP
load('real_ground_truth_decay_artifact.mat');
...
% Time 
time = eegdatadecay.step0.P001.EEG.times;
...
for idx = 1:length(id)
    for sx = 1:length(stepLevel)
        ...
        % Perform robust detrend
        EEGnew = tesa_robustdetrend(EEGnew,[10,1499],3,1);

        % Perform robust detrend
        EEGnew = tesa_robustdetrend(EEGnew,[-1500,-2],3,1);
...
for sx = 1:length(stepLevel)
    for idx = 1:length(id)
        eegraw.(['step',num2str(stepLevel(sx))])(:,:,idx) = mean(eegdatadecay.(['step',num2str(stepLevel(sx))]).(id{idx}).EEG.data,3);
    end
end
...
% Save the data
save([pathData,'real_decay_filter_butter_modified_detrend'],'rmse','stepLevel','eegraw','eegfilt','eegfiltband','time','tep_gt','time_gt');
