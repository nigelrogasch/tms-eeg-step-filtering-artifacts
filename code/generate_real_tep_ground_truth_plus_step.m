...
for sx = 1:length(stepLevel)
    ...
    for idx = 1:length(id)
        ...
        eegdatastep.(['step',num2str(stepLevel(sx))]).(id{idx}).EEG = eegdata.(id{idx}).EEG;
        eegdatastep.(['step',num2str(stepLevel(sx))]).(id{idx}).EEG.data = dataTemp;
        ...
        % Store the data
        eegdatastep.(['step',num2str(stepLevel(sx))]).(id{idx}).EEG.data = EEGnew.data;
...
for idx = 1:length(id)
    for sx = 1:length(stepLevel0)
        ...
        % Get the data
        EEGnew = eegdatastep.(['step',num2str(stepLevel0(sx))]).(id{idx}).EEG;
...
% Save the data
save([pathData,'real_ground_truth_step_artifact'],'eegdatastep','tep_gt0','time','id');
